  ...
  return (
    <div>
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3">
        ...
        <ExportCsvButton path={`/adminportal/customers/export?${new URLSearchParams(q ? { q } : {}).toString()}`} filename="customers.csv" />
      </div>
              ...
              {items.map((customer) => (
                <tr key={customer.id} className="border-b border-primary/5 last:border-0">
                  <td className="px-4 py-3">
                    <Link href={`/adminportal/customers/${customer.id}`} className="text-primary hover:underline">
                      {customer.full_name}
                    </Link>
                  </td>
