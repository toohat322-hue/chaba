...
import {
  createAdminFooterFeature,
  deleteAdminFooterFeature,
  getAdminFooterFeatures,
  updateAdminFooterFeature,
  type AdminFooterFeature,
  type FooterFeaturePayload,
} from "@/lib/api";
import { FooterFeatureForm } from "@/components/admin/FooterFeatureForm";

type Locale = "ar" | "fr" | "en";

export default function AdminFooterFeaturesPage() {
  const t = useTranslations("Admin");
  const locale = useLocale() as Locale;

  const [items, setItems] = useState<AdminFooterFeature[]>([]);
  ...
  async function handleCreate(payload: FooterFeaturePayload) {
    await createAdminFooterFeature(payload);
    setAdding(false);
    load();
  }

  async function handleUpdate(id: string, payload: FooterFeaturePayload) {
    await updateAdminFooterFeature(id, payload);
    setEditingId(null);
    load();
  }
  ...
  async function handleDelete(id: string) {
    ...
    await deleteAdminFooterFeature(id);
    load();
  }
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-h1 font-semibold text-primary">{t("footerFeaturesTitle")}</h1>
        {!adding && (
          <button
            type="button"
      ...
      {adding && (
        <div className="mb-6 rounded-2xl bg-white p-6 shadow-soft">
          <FooterFeatureForm onSubmit={handleCreate} onCancel={() => setAdding(false)} submitLabel={t("save")} />
        </div>
      )}
