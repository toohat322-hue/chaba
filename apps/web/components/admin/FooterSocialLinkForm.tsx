...
import type { AdminFooterSocialLink, FooterSocialLinkPayload } from "@/lib/api";
...
const PLATFORMS = ["instagram", "facebook", "tiktok", "snapchat", "twitter", "youtube", "whatsapp"] as const;

export function FooterSocialLinkForm({
  initial,
  onSubmit,
  onCancel,
  submitLabel,
}: {
  initial?: AdminFooterSocialLink;
  onSubmit: (payload: FooterSocialLinkPayload) => Promise<void>;
  onCancel?: () => void;
  submitLabel: string;
}) {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      await onSubmit({ platform, url, is_active: isActive, sort_order: Number(sortOrder) });
    } catch {
  ...
  return (
    <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-3 sm:grid-cols-4">
      <label className="block">
        <span className="mb-1 block text-caption text-ink/60">{t("platform")}</span>
