...
class SocialRedirectController extends Controller
{
    ...
    public function __invoke(Request $request, string $provider): RedirectResponse|JsonResponse
    {
        ...
        // TEMPORARY diagnostic wrap — every earlier attempt to read the real
        // exception from Render's Logs (even after switching LOG_STACK to
        // stderr specifically for this) turned up nothing next to the
        // failing request, and a from-scratch Google Cloud OAuth client +
        // publishing to Production made no difference either, which rules
        // out Google-side config entirely. Surfacing the exception directly
        // in the response is the only channel that's actually worked all
        // session (curl). Revert once the real cause is identified — never
        // ship exception details in a production response long-term.
        try {
            ...
            return response()->json([
                ...
                'trace' => collect($e->getTrace())->take(6)->map(fn ($frame) => ($frame['class'] ?? '').($frame['type'] ?? '').($frame['function'] ?? '').' @ '.basename($frame['file'] ?? '?').':'.($frame['line'] ?? '?'))->all(),
            ], 500);
        }
    }
